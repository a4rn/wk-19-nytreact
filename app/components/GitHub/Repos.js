...
var Repos = React.createClass({
      render: function() {
        ...
        var repos = this.props.repos.map(function(repo, index) {
              return ( < div > { /*It's important that we include the key. Otherwise React will throw an error*/ } < li className = "list-group-item"
                  key = {
                    index
                  } >

                  { /*Notice the funny && syntax again. This lets us display the a tag if it exists.*/ } { /*Look into double ampersand if confused. */ } {
                    repo.html_url && < h4 > < a href = {
                      repo.html_url
                    } > {
                      repo.name
                    } < /a></h4 >
